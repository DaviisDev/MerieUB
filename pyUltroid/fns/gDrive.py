...
class GDriveManager:
    def __init__(self):
        ...
        self.auth_token = mdB.get_key("GDRIVE_AUTH_TOKEN")
        self.folder_id = mdB.get_key("GDRIVE_FOLDER_ID")
    ...
    def _create_token_file(self, code: str = None):
        if code and self._flow:
            ...
            return mdB.set_key("GDRIVE_AUTH_TOKEN", str(open(self.token_file).read()))
        try:
            _auth_flow = OAuth2WebServerFlow(
                mdB.get_key("GDRIVE_CLIENT_ID")
                or "458306970678-jhfbv6o5sf1ar63o1ohp4c0grblp8qba.apps.googleusercontent.com",
                mdB.get_key("GDRIVE_CLIENT_SECRET")
    ...
    async def _upload_file(
        ...
        body = {
            "title": filename,
            "description": "Uploaded using Merie UserBot",
            "mimeType": mime_type,
        }
        if folder_id:
