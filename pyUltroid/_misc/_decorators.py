...
TAKE_EDITS = mdB.get_key("TAKE_EDITS")
...
def ultroid_cmd(
    ...
    def decor(dec):
        async def wrapp(ult):
            ...
            if only_devs and not mdB.get_key("I_DEV"):
                return await eod(
                    ult,
            ...
            try:
                ...
                await merie_bot.disconnect()
                await asyncio.sleep(fwerr.seconds + 10)
                await merie_bot.connect()
                await asst.send_message(
                    mdB.get_key("LOG_CHANNEL"),
        ...
        if manager and MANAGER:
            ...
            async def manager_cmd(ult):
                ...
                try:
                    ...
                    if chat := mdB.get_key("MANAGER_LOG"):
