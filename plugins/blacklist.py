...
from . import events, get_string, mdB, merie_bot, merie_cmd


@merie_cmd(pattern="blacklist( (.*)|$)", admins_only=True)
async def af(e):
...
@merie_cmd(pattern="remblacklist( (.*)|$)", admins_only=True)
async def rf(e):
    wrd = e.pattern_match.group(1).strip()
    chat = e.chat_id
    if not wrd:
...
@merie_cmd(pattern="listblacklist$", admins_only=True)
async def lsnote(e):
