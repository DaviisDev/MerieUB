...
async def hehe(args):
    ...
    if message.photo:
        photo = io.BytesIO()
        photo = await merie_bot.download_media(message.photo, photo)
    elif message.file and "image" in message.file.mime_type.split("/"):
        photo = io.BytesIO()
        await merie_bot.download_file(message.media.document, photo)
        if (
    ...
    if not mdB.get_key("language") or mdB.get_key("language") == "en":
    ...
    if photo:
        ...
        if not emoji:
            emoji = "🏵"
        if len(splat) == 3:
            pack = splat[2]  # User sent merie_both
            emoji = splat[1]
        elif len(splat) == 2:
        ...
        if (
            ...
            async with merie_bot.conversation("@Stickers") as conv:
                try:
                    await conv.send_message("/addsticker")
                except YouBlockedUserError:
                    LOGS.info("Unblocking @Stickers for kang...")
                    await merie_bot(functions.contacts.UnblockRequest("stickers"))
                    await conv.send_message("/addsticker")
                ...
                await conv.get_response()
                await merie_bot.send_read_acknowledge(conv.chat_id)
        else:
