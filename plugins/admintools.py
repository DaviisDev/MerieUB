...
from . import HNDLR, LOGS, eod, eor, get_string, inline_mention, types, merie_cmd
...
@merie_cmd(pattern="pin$", manager=True, require="pin_messages", fullsudo=True)
async def pin(msg):
    if not msg.is_reply:
...
@merie_cmd(pattern="purge( (.*)|$)", manager=True, require="delete_messages")
...
@merie_cmd(pattern="pinned", manager=True, groups_only=True)
