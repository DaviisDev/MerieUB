...
from . import events, get_string, mediainfo, mdB, merie_bot, merie_cmd
from ._inline import something


@merie_cmd(pattern="addnote( (.*)|$)", admins_only=True)
async def an(e):
    ...
    merie_bot.add_handler(notes, events.NewMessage())


@merie_cmd(pattern="remnote( (.*)|$)", admins_only=True)
async def rn(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    chat = e.chat_id
    if not wrd:
...
@merie_cmd(pattern="listnote$", admins_only=True)
async def lsnote(e):
