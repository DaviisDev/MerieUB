...
Owner_info_msg = mdB.get_key("BOT_INFO_START")
custom_info = True
...
**Message Forwards** - {mdB.get_key("PMBOT")}
...
async def own(event):
    msg = Owner_info_msg.format(
        mention=event.sender.mention, me=inline_mention(merie_bot.me)
    )
    if custom_info:
...
async def ultroid(event):
    ...
    if not keym.contains(event.sender_id) and event.sender_id not in owner_and_sudos():
        ...
        kak_uiw = mdB.get_key("OFF_START_LOG")
        if not kak_uiw or kak_uiw != True:
            ...
            await event.client.send_message(
                mdB.get_key("LOG_CHANNEL"), msg, buttons=buttons
            )
    if event.sender_id not in SUDO_M.fullsudos:
        ok = ""
        me = inline_mention(merie_bot.me)
        mention = inline_mention(event.sender)
        if args and args != "set":
            await get_stored_file(event, args)
        if not mdB.get_key("STARTMSG"):
            ...
            await event.reply(
                mdB.get_key("STARTMSG").format(me=me, mention=mention),
                file=mdB.get_key("STARTMEDIA"),
                buttons=[Button.inline("Info.", data="ownerinfo")]
                if Owner_info_msg
                else None,
...
async def botstat(event):
    ok = len(mdB.get_key("BOT_USERS") or [])
